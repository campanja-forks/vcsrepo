...
Puppet::Type.type(:vcsrepo).provide(:git, :parent => Puppet::Provider::Vcsrepo) do
  ...
  def latest
    if @resource.value(:revision) == 'master'
  ...
  def revision
    ...
    if tag_revision?(@resource.value(:revision))
      canonical = at_path { git_with_identity('show', @resource.value(:revision)).scan(/commit (.*)/)[0][0] }
    else
  ...
  def update_references
    at_path do
      ...
      git_with_identity('remote', 'prune', @resource.value(:remote))
